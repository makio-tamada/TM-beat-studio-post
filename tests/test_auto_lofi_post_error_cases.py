...
class TestAutoLofiPostErrorCases(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_jsonl_data = [
            {"type": "ambient", "music_prompt": "ambient music", "thumbnail_title": "Ambient Title", "image_prompts": ["ambient image"]},
            {"type": "jazz", "music_prompt": "jazz music", "thumbnail_title": "Jazz Title", "image_prompts": ["jazz image"]}
        ]
    ...
    def test_select_prompt_extracted_type_not_found(self, mock_requests, mock_validate):
        ...
        with patch.object(generator, '_extract_type_from_thumbnail', return_value="nonexistent_type"):
            jsonl_content = "\n".join([json.dumps(item) for item in self.test_jsonl_data])
    ...
    def test_generate_music_failure_handling(self, mock_requests, mock_validate):
        ...
        # 音楽生成を失敗させる
        with patch('auto_post.auto_lofi_post.piapi_music_generation', side_effect=Exception("Music generation failed")):
            with self.assertRaises(SystemExit):
    ...
    def test_combine_audio_tracks_failure_handling(self, mock_requests, mock_validate):
        ...
        # 音声結合を失敗させる
        with patch('auto_post.auto_lofi_post.combine_audio', side_effect=Exception("Audio combination failed")):
            with self.assertRaises(SystemExit):
    ...
    def test_generate_thumbnail_failure_handling(self, mock_requests, mock_validate):
        ...
        # サムネイル生成を失敗させる
        with patch('auto_post.auto_lofi_post.thumbnail_generation', side_effect=Exception("Thumbnail generation failed")):
            with self.assertRaises(SystemExit):
    ...
    def test_generate_metadata_failure_handling(self, mock_requests, mock_validate):
        ...
        # メタデータ生成を失敗させる
        with patch('auto_post.auto_lofi_post.create_metadata', side_effect=Exception("Metadata generation failed")):
            with self.assertRaises(SystemExit):
                generator.generate_metadata("test_tracks.json")
    ...
    def test_generate_video_failure_handling(self, mock_requests, mock_validate):
        ...
        # 動画生成を失敗させる
        with patch('auto_post.auto_lofi_post.create_video', side_effect=Exception("Video generation failed")):
            with self.assertRaises(SystemExit):
    ...
    def test_upload_to_youtube_failure_handling(self, mock_requests, mock_validate):
        ...
        # YouTubeアップロードを失敗させる
        with patch('auto_post.auto_lofi_post.upload_video_to_youtube', side_effect=Exception("Upload failed")):
            with self.assertRaises(SystemExit):
                generator.upload_to_youtube("test_video.mp4", "test_thumbnail.jpg", "test_metadata.json")
    ...
    def test_thumbnail_title_list_handling(self, mock_requests, mock_validate):
        ...
        with patch('builtins.open', mock_open(read_data=jsonl_content)):
            ...
            self.assertIn(generator.selected_prompt["thumbnail_title"], ["Title 1", "Title 2", "Title 3"])
