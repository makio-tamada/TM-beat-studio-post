...
class Config:
    ...
    # ファイルパス設定
    JSONL_PATH = Path(
        os.getenv("JSONL_PATH", "src/auto_post/lofi_type_with_variations.jsonl")
    )
    OPENING_VIDEO_PATH = Path(
        os.getenv("OPENING_VIDEO_PATH", "src/auto_post/openning.mov")
    )
    AMBIENT_DIR = Path(os.getenv("AMBIENT_DIR", "data/ambient"))
    CLIENT_SECRETS_PATH = Path(
        os.getenv("CLIENT_SECRETS_PATH", "src/auto_post/client_secrets.json")
    )
    POST_DETAIL_PATH = Path(
        os.getenv("POST_DETAIL_PATH", "src/auto_post/post_detail.txt")
    )
    ...
    def validate_config(cls):
        ...
        if not cls.STOCK_AUDIO_BASE_DIR.exists():
            print(
                f"警告: 音声ストックディレクトリが存在しません: {cls.STOCK_AUDIO_BASE_DIR}"
            )

        if not cls.STOCK_IMAGE_BASE_DIR.exists():
            print(
                f"警告: 画像ストックディレクトリが存在しません: {cls.STOCK_IMAGE_BASE_DIR}"
            )
        ...
        if not cls.OPENING_VIDEO_PATH.exists():
            print(
                f"警告: オープニング動画ファイルが存在しません: {cls.OPENING_VIDEO_PATH}"
            )
        ...
        if not cls.CLIENT_SECRETS_PATH.exists():
            print(
                f"警告: クライアントシークレットファイルが存在しません: {cls.CLIENT_SECRETS_PATH}"
            )
