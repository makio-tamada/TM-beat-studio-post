...
class TestConfig(unittest.TestCase):
    ...
    def test_config_with_custom_env_vars(self, mock_getenv):
        ...
        # アサーション
        self.assertEqual(
            auto_post.config.Config.SLACK_WEBHOOK_URL, "https://hooks.slack.com/test"
        )
        self.assertEqual(
            str(auto_post.config.Config.STOCK_AUDIO_BASE_DIR), "/custom/audio"
        )
        self.assertEqual(
            str(auto_post.config.Config.STOCK_IMAGE_BASE_DIR), "/custom/image"
        )
        self.assertEqual(str(auto_post.config.Config.JSONL_PATH), "/custom/lofi.jsonl")
        self.assertEqual(
            str(auto_post.config.Config.OPENING_VIDEO_PATH), "/custom/opening.mov"
        )
        self.assertEqual(str(auto_post.config.Config.AMBIENT_DIR), "/custom/ambient")
        self.assertEqual(
            str(auto_post.config.Config.CLIENT_SECRETS_PATH),
            "/custom/client_secrets.json",
        )
        self.assertEqual(
            str(auto_post.config.Config.POST_DETAIL_PATH), "/custom/post_detail.txt"
        )
        self.assertEqual(
            auto_post.config.Config.COMBINED_AUDIO_FILENAME, "custom_audio.mp3"
        )
        self.assertEqual(
            auto_post.config.Config.TRACKS_INFO_FILENAME, "custom_tracks.json"
        )
        self.assertEqual(
            auto_post.config.Config.METADATA_FILENAME, "custom_metadata.json"
        )
        self.assertEqual(
            auto_post.config.Config.FINAL_VIDEO_FILENAME, "custom_video.mp4"
        )
    ...
    def test_config_default_values(self):
        ...
        with patch("auto_post.config.os.getenv", side_effect=mock_getenv):
            ...
            self.assertEqual(
                str(auto_post.config.Config.STOCK_AUDIO_BASE_DIR), "/tmp/music/lofi"
            )
            self.assertEqual(
                str(auto_post.config.Config.STOCK_IMAGE_BASE_DIR), "/tmp/image"
            )
            self.assertEqual(
                str(auto_post.config.Config.JSONL_PATH),
                "src/auto_post/lofi_type_with_variations.jsonl",
            )
            self.assertEqual(
                str(auto_post.config.Config.OPENING_VIDEO_PATH),
                "src/auto_post/openning.mov",
            )
            self.assertEqual(str(auto_post.config.Config.AMBIENT_DIR), "data/ambient")
            self.assertEqual(
                str(auto_post.config.Config.CLIENT_SECRETS_PATH),
                "src/auto_post/client_secrets.json",
            )
            self.assertEqual(
                str(auto_post.config.Config.POST_DETAIL_PATH),
                "src/auto_post/post_detail.txt",
            )
            self.assertEqual(
                auto_post.config.Config.COMBINED_AUDIO_FILENAME, "combined_audio.mp3"
            )
            self.assertEqual(
                auto_post.config.Config.TRACKS_INFO_FILENAME, "tracks_info.json"
            )
            self.assertEqual(auto_post.config.Config.METADATA_FILENAME, "metadata.json")
            self.assertEqual(
                auto_post.config.Config.FINAL_VIDEO_FILENAME, "final_video.mp4"
            )
