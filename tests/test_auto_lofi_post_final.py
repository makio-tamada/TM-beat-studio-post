...
class TestAutoLofiPostFinal(unittest.TestCase):
    ...
    def test_select_prompt_success(self, mock_validate_config):
        ...
        with patch(
            "builtins.open",
            new=mock_open(
                read_data='{"type": "sad", "music_prompt": "melancholic piano", "thumbnail_title": "Sad Lo-Fi", "ambient": "rain.mp3"}'
            ),
        ):
            with patch("builtins.print"):
    ...
    def test_generate_music_success(self, mock_validate_config):
        """音楽生成成功時のテスト"""
        generator = LofiPostGenerator(self.args)
        generator.selected_prompt = {
            "type": "sad",
            "music_prompt": "sad lo-fi",
            "ambient": "rain.mp3",
        }
    ...
    def test_upload_to_youtube_skip(self, mock_validate_config):
        ...
        with patch("builtins.print"):
            result = generator.upload_to_youtube(
                "video.mp4", "thumbnail.png", "metadata.json"
            )
