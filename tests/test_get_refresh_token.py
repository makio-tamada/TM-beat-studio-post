...
from auto_post.get_refresh_token import get_refresh_token, SCOPES
...
class TestGetRefreshToken(unittest.TestCase):
    ...
    def test_get_refresh_token_success(self, mock_path, mock_file, mock_flow_class, mock_config):
        """リフレッシュトークン取得成功時のテスト"""
        # モックの設定
    ...
    def test_get_refresh_token_client_secrets_not_found(self, mock_print, mock_config):
        ...
        # アサーション
        mock_print.assert_any_call(f"エラー: {mock_client_secrets_path} が見つかりません")
        mock_print.assert_any_call("Google Cloud Consoleからclient_secrets.jsonをダウンロードして、このディレクトリに配置してください")
    
    @patch('auto_post.config.Config')
    @patch('auto_post.get_refresh_token.InstalledAppFlow')
    @patch('builtins.open', new_callable=mock_open)
    @patch('auto_post.get_refresh_token.Path')
    def test_get_refresh_token_flow_exception(self, mock_path, mock_file, mock_flow_class, mock_config):
        """認証フローで例外が発生した場合のテスト"""
        # モックの設定
        mock_client_secrets_path = Mock()
        mock_client_secrets_path.exists.return_value = True
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('auto_post.get_refresh_token.Path')
    def test_get_refresh_token_file_write_exception(self, mock_path, mock_file, mock_flow_class, mock_config):
