...
* Read `tracks_info.json` (generated by combine_audio.py) to build a timestamped track‑list
...
def main():
    ...
    raw_desc = call_openai(description_prompt(lofi_type, music_prompt), 200, args.temperature)
...
def create_metadata(output_dir: str, tracks_json: str, lofi_type: str, music_prompt: str, api_url: str, temperature: float) -> Path:
    ...
    raw_desc = call_openai(description_prompt(lofi_type, music_prompt), 200, temperature)
    description = raw_desc.replace("Description:", "").strip()
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    base = f"{lofi_type.lower()}_{timestamp}"
    (output_dir / f"title.txt").write_text(title, encoding="utf-8")
    (output_dir / f"description.txt").write_text(description, encoding="utf-8")
    (output_dir / f"tracklist.txt").write_text(tracklist, encoding="utf-8")
    (output_dir / f"post_detail.txt").write_text(post_detail, encoding="utf-8")
    ...
    with open(output_dir / f"metadata.json", "w", encoding="utf-8") as f:
        json.dump(meta, f, ensure_ascii=False, indent=2)
    ...
    return output_dir / f"metadata.json"
