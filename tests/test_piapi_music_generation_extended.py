...
class TestPiapiMusicGenerationExtended(unittest.TestCase):
    ...
    def setUp(self):
        ...
        test_data = [
            {
                "type": "sad",
                "music_prompt": "melancholic lo-fi beats",
    ...
    def test_generate_unique_filename_no_conflict(
        self, mock_fetch_title, mock_get_existing
    ):
        """ファイル名生成（重複なし）のテスト"""
        ...
        result = generate_unique_filename(
            "Test Track", str(self.temp_path), "test prompt"
        )
    ...
    @patch("auto_post.piapi_music_generation.get_existing_filenames")
    @patch("auto_post.piapi_music_generation.fetch_track_title")
    def test_generate_unique_filename_with_conflict(
        self, mock_fetch_title, mock_get_existing
    ):
        """ファイル名生成（重複あり）のテスト"""
    ...
    @patch("auto_post.piapi_music_generation.get_existing_filenames")
    @patch("auto_post.piapi_music_generation.fetch_track_title")
    def test_generate_unique_filename_fallback_to_counter(
        self, mock_fetch_title, mock_get_existing
    ):
    ...
    def test_choose_random_prompt(self, mock_jsonl_path):
        ...
        with patch(
            "builtins.open",
            mock_open(
                read_data=json.dumps({"type": "sad", "music_prompt": "test"}) + "\n"
            ),
        ):
    ...
    def test_wait_for_task_failed(self, mock_sleep, mock_get):
        ...
        mock_response = Mock()
        mock_response.json.return_value = {
            "data": {"status": "failed", "error": "Task failed"}
        }
        mock_response.raise_for_status.return_value = None
        mock_get.return_value = mock_response
    ...
    def test_extract_audio_url_songs(self):
        ...
        task_data = {
            "output": {"songs": [{"song_path": "http://example.com/audio.mp3"}]}
        }
