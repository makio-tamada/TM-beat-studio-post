...
def get_authenticated_service():
    ...
    # 必要な環境変数のチェック
    required_env_vars = ['GOOGLE_REFRESH_TOKEN', 'GOOGLE_CLIENT_ID', 'GOOGLE_CLIENT_SECRET']
    ...
    try:
        ...
        if not client_secrets_path.exists():
            raise Exception(f"client_secrets.jsonが見つかりません: {client_secrets_path}")
...
def upload_video(video_path, title, description, category_id="10", privacy_status="private", 
                 tags=None, made_for_kids=False, default_language="en", license_type="youtube"):
    """
    動画をYouTubeにアップロード
...
def main():
    ...
    parser.add_argument('--video', type=str, required=True, help='アップロードする動画ファイルのパス')
    ...
    parser.add_argument('--privacy', type=str, default="private", choices=["private", "public", "unlisted"],
...
def upload_video_to_youtube(video_path: Path, thumbnail_path: Path, metadata_path: Path, privacy: str = "private", tags: list = None):
    # メタデータを読み込む
