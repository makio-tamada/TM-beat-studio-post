...
class TestThumbnailGeneration(unittest.TestCase):
    ...
    def test_create_thumbnail(self, mock_enhance, mock_font, mock_draw, mock_image):
        ...
        mock_img.resize.assert_called_once_with((THUMB_WIDTH, THUMB_HEIGHT), mock_image.LANCZOS)
    ...
    @patch('auto_post.thumbnail_generation.ImageFont')
    @patch('auto_post.thumbnail_generation.ImageEnhance')
    def test_create_thumbnail_multiline_title(self, mock_enhance, mock_font, mock_draw, mock_image):
        """複数行タイトルのサムネイル作成テスト"""
        # モックの設定
        mock_img = Mock()
        mock_image.open.return_value.convert.return_value = mock_img
    ...
    def test_thumbnail_generation_success(self, mock_create_thumb, mock_ensure_font, mock_pipeline):
        """サムネイル生成成功時のテスト"""
        # モックの設定
        mock_pipe = Mock()
        mock_image = Mock()
    ...
    @patch('auto_post.thumbnail_generation.ensure_font')
    @patch('auto_post.thumbnail_generation.create_thumbnail')
    def test_thumbnail_generation_safe_filename(self, mock_create_thumb, mock_ensure_font, mock_pipeline):
        """安全なファイル名生成のテスト"""
        # モックの設定
        mock_pipe = Mock()
        mock_image = Mock()
    ...
    @patch('auto_post.thumbnail_generation.ensure_font')
    @patch('auto_post.thumbnail_generation.create_thumbnail')
    def test_main_function(self, mock_create_thumb, mock_ensure_font, mock_load_prompt, mock_pipeline):
        """main関数のテスト"""
        # モックの設定
    ...
    def test_device_selection(self, mock_torch):
        ...
        with patch('auto_post.thumbnail_generation.DiffusionPipeline') as mock_pipeline:
            ...
            with patch('auto_post.thumbnail_generation.ensure_font'), \
                 patch('auto_post.thumbnail_generation.create_thumbnail'), \
