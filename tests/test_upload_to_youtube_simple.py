...
class TestUploadToYouTubeSimple(unittest.TestCase):
    ...
    def test_upload_video_to_youtube_success(self, mock_build, mock_flow, mock_upload_video, mock_validate_config):
        ...
        mock_service.videos.return_value.insert.return_value.execute.return_value = mock_response
    ...
    @patch('auto_post.upload_to_youtube.InstalledAppFlow')
    @patch('auto_post.upload_to_youtube.build')
    def test_upload_video_to_youtube_upload_error(self, mock_build, mock_flow, mock_validate_config):
        """アップロードエラー時のテスト"""
        # モックの設定
        mock_credentials = MagicMock()
        mock_flow.return_value.run_local_server.return_value = mock_credentials
        ...
        # アップロードエラーのモック
        mock_service.videos.return_value.insert.return_value.execute.side_effect = Exception("Upload failed")
    ...
    @patch('auto_post.upload_to_youtube.InstalledAppFlow')
    @patch('auto_post.upload_to_youtube.build')
    def test_upload_video_to_youtube_metadata_error(self, mock_build, mock_flow, mock_validate_config):
        """メタデータ読み込みエラー時のテスト"""
        # モックの設定
        mock_credentials = MagicMock()
        mock_flow.return_value.run_local_server.return_value = mock_credentials
    ...
    @patch('auto_post.upload_to_youtube.InstalledAppFlow')
    @patch('auto_post.upload_to_youtube.build')
    def test_upload_video_to_youtube_authentication_error(self, mock_build, mock_flow, mock_validate_config):
        """認証エラー時のテスト"""
        # 認証エラーのモック
        mock_flow.return_value.run_local_server.side_effect = Exception("Authentication failed")
    ...
    @patch('auto_post.upload_to_youtube.InstalledAppFlow')
    @patch('auto_post.upload_to_youtube.build')
    def test_upload_video_to_youtube_different_privacy_settings(self, mock_build, mock_flow, mock_upload_video, mock_validate_config):
        """異なる公開設定でのアップロードテスト"""
        # モックの設定
        mock_credentials = MagicMock()
        mock_flow.return_value.run_local_server.return_value = mock_credentials
