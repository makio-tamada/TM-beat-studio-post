...
class LofiPostGenerator:
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        self.newly_generated_files: List[Path] = []  # 新規生成したファイルのリストを保持
    ...
    def setup(self) -> None:
        ...
        self.send_slack_notification(f"🚀 Lo-Fi音楽生成処理を開始します\n日付: {self.args.output_dir}")
        ...
        try:
            if self.args.skip_type_selection:
                ...
                with open(jsonl_path, "r", encoding="utf-8") as f:
                    ...
                    selected = next((sample for sample in samples if 'sad' in sample["type"].lower()), None)
                    ...
                    if isinstance(self.selected_prompt["thumbnail_title"], list):
                        self.selected_prompt["thumbnail_title"] = random.choice(self.selected_prompt["thumbnail_title"])
    ...
    def _select_specific_prompt(self) -> None:
        ...
        with open(jsonl_path, "r", encoding="utf-8") as f:
            lines = f.readlines()
            samples = [json.loads(line) for line in lines if line.strip()]
            selected = next((sample for sample in samples if sample["type"].lower() == self.args.lofi_type.lower()), None)
            
            if not selected:
                raise ValueError(f"指定されたタイプ '{self.args.lofi_type}' が見つかりません")
    ...
    def _select_random_prompt(self) -> None:
        ...
        with open(jsonl_path, "r", encoding="utf-8") as f:
            ...
            # image_promptsから1つをランダム選択
            if "image_prompts" in self.selected_prompt and self.selected_prompt["image_prompts"]:
                self.selected_image_prompt = random.choice(self.selected_prompt["image_prompts"])
            else:
                # 後方互換性のため、image_promptも確認
                self.selected_image_prompt = self.selected_prompt.get("image_prompt", "")

    def _print_selected_prompt(self) -> None:
        """選択されたプロンプトを表示"""
        print(f"==> Selected type: {self.selected_prompt['type']}")
        print(f'==> Generating music for prompt: "{self.selected_prompt["music_prompt"]}"')
        print(f'==> Generating thumbnail for prompt: "{self.selected_prompt["thumbnail_title"]}"')
    ...
    def _load_music_from_stock(self, stock_audio_dir: Path, target_duration: int) -> None:
        """ストックから音楽を読み込む"""
        print(f"==> 音楽ストックから音楽を読み込みます")
    ...
    def _select_music_files(self, stock_files: List[Path], target_duration: int) -> List[Path]:
        """音楽ファイルを選択（改善版：より正確に目標時間に近づける）"""
        music_files_with_duration = [
            (file, len(AudioSegment.from_mp3(file)) / 1000)
        ...
        while total_duration < min_target and music_files_with_duration and attempts < max_attempts:
            attempts += 1
            ...
            suitable_files = [
                (file, duration) for file, duration in music_files_with_duration
        ...
        print(f"==> 選択完了: 目標 {target_duration}秒 → 実際 {total_duration:.1f}秒 ({total_duration/60:.1f}分)")
    ...
    def _copy_selected_files_to_output(self, selected_files: List[Path]) -> None:
        ...
        for file in selected_files:
            ...
            print(f"==> ストックから音楽をコピーしました: {new_name} (長さ: {duration:.1f}秒)")
        
        print(f"==> 合計長さ: {total_duration:.1f}秒")
        self.send_slack_notification(f"🎵 ストックから音楽を読み込みました\n合計長さ: {total_duration:.1f}秒")
    ...
    def combine_audio_tracks(self) -> Tuple[str, str]:
        ...
        if self.args.skip_audio_combine:
            ...
            if not output_mp3 or not tracks_json:
                error_msg = "音声結合をスキップしましたが、必要なファイルが見つかりません"
                self.send_slack_notification(error_msg, is_error=True)
                print(f"==> {error_msg}")
                sys.exit(1)
                
            print(f"==> 既存のファイルを使用します: {output_mp3.name}, {tracks_json.name}")
            elapsed_time = time.time() - start_time
    ...
    def generate_thumbnail(self) -> Tuple[str, str]:
        ...
        if self.args.skip_thumbnail_gen:
            ...
            if not thumbnail_path:
                error_msg = "サムネイル生成をスキップしましたが、必要なファイルが見つかりません"
                self.send_slack_notification(error_msg, is_error=True)
                print(f"==> {error_msg}")
                sys.exit(1)
    ...
    def generate_metadata(self, tracks_json_path: str) -> str:
        ...
        if self.args.skip_metadata_gen:
            ...
            if not metadata_path:
                error_msg = "メタデータ生成をスキップしましたが、必要なファイルが見つかりません"
                self.send_slack_notification(error_msg, is_error=True)
                print(f"==> {error_msg}")
                sys.exit(1)
    ...
    def generate_video(self, image_path: str, output_mp3_path: str) -> Optional[str]:
        ...
        if self.args.skip_video_gen:
            ...
            if not video_path:
                error_msg = "動画生成をスキップしましたが、必要なファイルが見つかりません"
                self.send_slack_notification(error_msg, is_error=True)
                print(f"==> {error_msg}")
                sys.exit(1)
    ...
    def upload_to_youtube(self, video_path: str, thumbnail_path: str, metadata_path: str) -> None:
        """YouTubeにアップロード"""
        start_time = time.time()
        print("\n=== YouTubeにアップロード ===")
        if self.args.skip_upload:
            ...
            elapsed_time = time.time() - start_time
            print(f"==> YouTubeアップロードスキップ完了 (処理時間: {elapsed_time:.2f}秒)")
            return
...
def parse_args() -> argparse.Namespace:
    """コマンドライン引数を解析"""
    parser = argparse.ArgumentParser(description="自動でyoutubeに音楽を投稿するプログラム")

    # 共通
    common_group = parser.add_argument_group("共通")
    common_group.add_argument("--output_dir", type=str, default=datetime.now().strftime("%Y%m%d"), help="出力ディレクトリ")
    common_group.add_argument("--jsonl_path", type=str, help="jsonlファイルのパス（未指定時は環境変数JSONL_PATHを使用）")
    common_group.add_argument("--lofi_type", type=str, help="指定するLo-Fiタイプ（指定がない場合はランダム）")
    common_group.add_argument("--skip_type_selection", action="store_true", help="Lo-Fiタイプの選択をスキップする")

    # 音楽
    music_group = parser.add_argument_group("音楽")
    music_group.add_argument("--target_duration_sec", type=int, default=60*12*6, help="音楽の長さ（フェード処理による短縮を考慮して余裕を持たせた時間）")
    music_group.add_argument("--ambient_dir", type=str, help="環境音ファイルのディレクトリ（未指定時は環境変数AMBIENT_DIRを使用）")
    music_group.add_argument("--skip_music_gen", action="store_true", help="音楽生成をスキップする")
    music_group.add_argument("--skip_audio_combine", action="store_true", help="音声結合をスキップする")

    # サムネイル
    thumbnail_group = parser.add_argument_group("サムネイル")
    thumbnail_group.add_argument("--skip_thumbnail_gen", action="store_true", help="サムネイル生成をスキップする")

    # メタデータ
    metadata_group = parser.add_argument_group("メタデータ")
    metadata_group.add_argument("--temperature", type=float, default=0.7, help="生成温度")
    metadata_group.add_argument("--skip_metadata_gen", action="store_true", help="メタデータ生成をスキップする")
    
    # 動画
    video_group = parser.add_argument_group("動画")
    video_group.add_argument("--skip_video_gen", action="store_true", help="動画生成をスキップする")
    
    # アップロード
    upload_group = parser.add_argument_group("アップロード")
    upload_group.add_argument("--privacy", type=str, default="public", choices=["private", "public", "unlisted"], help="公開設定")
    upload_group.add_argument("--tags", nargs='+', default=["lofi", "music", "relax", "sleep", "study", "work", "chill", "ambient", "background", "background music", "background sound", "background noise", "background music", "background sound", "background noise"], help="タグ")
    upload_group.add_argument("--skip_upload", action="store_true", help="アップロードをスキップする")
