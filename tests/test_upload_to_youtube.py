...
class TestUploadToYoutube(unittest.TestCase):
    ...
    def test_get_authenticated_service_with_refresh_token(
        self, mock_build, mock_credentials, mock_getenv
    ):
        """リフレッシュトークンでの認証テスト"""
        # 環境変数を設定
    ...
    def test_get_authenticated_service_new_auth(
        self, mock_config, mock_build, mock_flow, mock_credentials, mock_getenv
    ):
        """新規認証のテスト"""
        # リフレッシュトークン認証を失敗させる
        mock_getenv.side_effect = lambda key: {
    ...
    @patch("auto_post.config.Config")
    @patch("builtins.open", new_callable=mock_open)
    def test_upload_video_to_youtube_upload_failure(
        self,
        mock_file,
        mock_config,
        mock_upload_video,
