...
class TestPiapiMusicGeneration(unittest.TestCase):
    ...
    def test_wait_for_task_completed(self, mock_get):
        ...
        mock_response.json.return_value = {
            ...
            "output": {
                "songs": [
                    {"duration": 120, "url": "https://example.com/audio.mp3"}
                ]
            }
        }
    ...
    def test_piapi_music_generation_success(self, mock_extract_url, mock_filename, mock_title, mock_download, mock_wait, mock_create):
        """音楽生成全体の成功フローテスト"""
        # モックの設定
