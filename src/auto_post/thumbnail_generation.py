...
LOBSTER_FONT_URL = os.getenv('LOBSTER_FONT_URL', "https://github.com/google/fonts/raw/main/ofl/lobster/Lobster-Regular.ttf")
FONT_DIR = Path(os.getenv('FONT_DIR', 'src/auto_post/fonts'))
FONT_DIR.mkdir(parents=True, exist_ok=True)
LOBSTER_FONT_PATH = Path(os.getenv('LOBSTER_FONT_PATH', str(FONT_DIR / "Lobster-Regular.ttf")))
...
def ensure_font() -> str:
    """
    Download Lobster font once and return local path.
    """
    if not LOBSTER_FONT_PATH.exists():
        print("==> Downloading Lobster font...")
        r = requests.get(LOBSTER_FONT_URL, timeout=30)
        r.raise_for_status()
        with open(LOBSTER_FONT_PATH, "wb") as f:
            f.write(r.content)
        print(f"==> Saved font to {LOBSTER_FONT_PATH}")
    return str(LOBSTER_FONT_PATH)

def create_thumbnail(bg_image_path: str, title: str, output_path: str, font_path: str, font_size: int = 180):
    """
...
def thumbnail_generation(output_dir: str, lofi_type: str, prompt: str, thumb_title: str) -> tuple[str, str]:
    # デバイス設定
    device = "mps" if torch.backends.mps.is_available() else "cuda" if torch.cuda.is_available() else "cpu"
    print(f"==> Using device: {device}")
    ...
    # 保存 ― 余分な文字 ( / \ : * ? " < > | ) などを安全な '_' に置換
    safe_type = re.sub(r'[^0-9A-Za-z_\-]+', '_', lofi_type.lower())
    filename = f"{safe_type}_{THUMB_WIDTH}x{THUMB_HEIGHT}_{datetime.datetime.now().strftime('%Y%m%d')}.png"
    out_path = os.path.join(output_dir, filename)
    image.save(out_path)
    print(f"==> Saved image to {out_path}")
    ...
    # Create thumbnail with title text
    font_path = ensure_font()
    thumb_name = filename.replace(".png", f"_{datetime.datetime.now().strftime('%Y%m%d')}_thumb.png")
    thumb_out = os.path.join(output_dir, thumb_name)
    create_thumbnail(
        bg_image_path=out_path,
        title=thumb_title,
